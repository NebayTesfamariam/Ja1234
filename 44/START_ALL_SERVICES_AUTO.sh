#!/bin/bash
# ===============================
# START ALL SERVICES WITH PASSWORD PROMPT
# ===============================
...
# Function to run sudo command with password prompt
run_sudo() {
    local cmd="$1"
...
if pgrep -x "mysqld" > /dev/null 2>&1; then
    ...
    if run_sudo "/Applications/XAMPP/xamppfiles/bin/mysql.server start"; then
        sleep 2
        if pgrep -x "mysqld" > /dev/null 2>&1; then
            echo "   ✅ MySQL gestart"
        else
        ...
        echo "   ❌ MySQL start mislukt (password required)"
    fi
fi
...
if pgrep -x "httpd" > /dev/null 2>&1; then
    ...
    if run_sudo "/Applications/XAMPP/xamppfiles/bin/httpd -k start"; then
        sleep 2
        if pgrep -x "httpd" > /dev/null 2>&1; then
            echo "   ✅ Apache gestart"
        else
        ...
        echo "   ❌ Apache start mislukt (password required)"
    fi
fi
...
if pgrep -f "dns_whitelist_server.py" > /dev/null 2>&1; then
    ...
    if run_sudo "lsof -i :53" > /dev/null 2>&1; then
        echo "   ⚠️  Poort 53 al in gebruik, stoppen oude DNS server..."
        run_sudo "pkill -f dns_whitelist_server.py" 2>/dev/null
        sleep 2
    fi
    
    echo "   ⚠️  DNS server starten..."
    
    # Start DNS server in background using osascript
    osascript -e "do shell script \"cd '$SCRIPT_DIR' && nohup python3 dns_whitelist_server.py > logs/dns_server.log 2>&1 &\" with administrator privileges" 2>/dev/null
